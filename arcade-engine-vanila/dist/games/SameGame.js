...
//#region src/games/SameGame.ts
var SameGame = class extends GameModel {
    grid=[];
    constructor(audio) {
        super(10, 10, "samegame", audio);
    }
    start() {
        this.level = 1;
        this.startLevel();
    ...
    handleInput(action) {
        ...
        while (stack.length) {
            ...
            [ [ 0, 1 ], [ 0, -1 ], [ 1, 0 ], [ -1, 0 ] ].forEach(d => {
                ...
                if (nx >= 0 && nx < this.width && ny >= 0 && ny < this.height && this.grid[nx][ny] === color) stack.push({
                    x: nx,
                    y: ny
                });
            });
        }
        if (group.length > 1) this.removeGroup(group);
    }
    async removeGroup(group) {
        ...
        group.forEach(p => {
            this.effects$.next({
                ...
                x: p.x,
                y: p.y,
                color: this.getColor(this.grid[p.x][p.y]),
                style: "PUFF"
            });
            this.grid[p.x][p.y] = 0;
        });
    ...
    checkWin() {
        if (this.grid.every(col => col.every(c => c === 0))) this.handleWin();
    }
    handleWin() {
    ...
    emit() {
        const items = [];
        for (let x = 0; x < this.width; x++) for (let y = 0; y < this.height; y++) if (this.grid[x][y] !== 0) items.push({
            id: `sg_${x}_${y}`,
            x: x,
            y: y,
            type: this.grid[x][y]
