...
//#region src/games/LightsOut.ts
var LightsCopyGame = class extends GameModel {
    grid=[];
    constructor(audio) {
    ...
    startLevel() {
        ...
        for (let i = 0; i < clicks; i++) this.toggle(Math.floor(Math.random() * size), Math.floor(Math.random() * size), true);
        this.emit();
    ...
    checkWin() {
        if (this.grid.every(col => col.every(c => !c))) this.handleWin();
    }
    handleWin() {
    ...
    emit() {
        const items = [];
        for (let x = 0; x < this.width; x++) for (let y = 0; y < this.height; y++) items.push({
            id: `c_${x}_${y}`,
            x: x,
            y: y,
