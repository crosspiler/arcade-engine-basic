...
export default defineConfig(({ mode, command }) => {
  // We need to call the default config function to get the base object
  const baseConfig = defaultConfig({ mode, command, isSsrBuild: false });
  ...
  const mergedBuildOptions = {
    ...
    rollupOptions: {
      ...
      ...separateBuildOptions.build.rollupOptions,
    },
  };
  return { ...baseConfig, build: mergedBuildOptions };
});