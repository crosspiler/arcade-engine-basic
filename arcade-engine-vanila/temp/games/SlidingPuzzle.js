...
//#region src/games/SlidingPuzzle.ts
var SlidingPuzzle = class extends GameModel {
    tiles=[];
    constructor(audio) {
        super(4, 4, "sliding", audio);
    }
    start() {
        this.level = 1;
        this.startLevel();
    ...
    startLevel() {
        ...
        for (let i = 1; i < count; i++) this.tiles.push({
            v: i,
            x: (i - 1) % size,
            y: size - 1 - Math.floor((i - 1) / size)
        });
        let lx = size - 1, ly = 0;
        const moves = 50 * this.level;
        for (let i = 0; i < moves; i++) {
            const neighbors = [];
            if (lx > 0) neighbors.push({
                x: lx - 1,
                y: ly
            });
            if (lx < size - 1) neighbors.push({
                x: lx + 1,
                y: ly
            });
            if (ly > 0) neighbors.push({
                x: lx,
                y: ly - 1
            });
            if (ly < size - 1) neighbors.push({
                x: lx,
                y: ly + 1
            });
            const n = neighbors[Math.floor(Math.random() * neighbors.length)];
            const t = this.tiles.find(t$1 => t$1.x === n.x && t$1.y === n.y);
            if (t) {
                t.x = lx;
                t.y = ly;
                lx = n.x;
    ...
    checkWin() {
        if (this.tiles.every(t => t.x === (t.v - 1) % this.width && t.y === this.height - 1 - Math.floor((t.v - 1) / this.width))) this.handleWin();
    }
    handleWin() {
        this.status$.next("SOLVED!");
        this.updateScore(1e3 * this.level);
