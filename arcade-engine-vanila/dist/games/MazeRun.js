...
//#region src/games/MazeRun.ts
var MazeRun = class extends GameModel {
    grid=[];
    player={
        x: 1,
        y: 1
    ...
    generateLevel() {
        this.grid = Array(this.width).fill(null).map(() => Array(this.height).fill(1));
        const stack = [ {
            x: 1,
            y: 1
        } ];
        ...
        while (stack.length) {
            ...
            [ [ 0, 2 ], [ 0, -2 ], [ 2, 0 ], [ -2, 0 ] ].forEach(d => {
                ...
                if (nx > 0 && nx < this.width - 1 && ny > 0 && ny < this.height - 1 && this.grid[nx][ny] === 1) neighbors.push({
                    x: nx,
                    y: ny,
                    dx: d[0] / 2,
                    dy: d[1] / 2
    ...
    handleInput(action) {
        ...
        if (nx >= 0 && nx < this.width && ny >= 0 && ny < this.height && this.grid[nx][ny] === 0) {
            ...
            this.emit();
            if (nx === this.exit.x && ny === this.exit.y) this.handleWin();
        }
    }
    handleWin() {
